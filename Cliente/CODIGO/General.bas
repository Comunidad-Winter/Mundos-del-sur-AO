...
Sub Main()
    ...
    #If Desarrollo = 0 Then
        If Not LeerInt(App.path & "\INIT\Launcher.ini") = 1 Then
            MsgBox "Debes abrir el juego desde Mundos del Sur.exe"
            End
        End If
    #End If
    Call GuardarInt(App.path & "\INIT\Launcher.ini", 0)
    #If Desarrollo = 0 Then
        Ip = "mdsao.ddns.net" '"mdsao.ddns.net" ''frmCargando.Inet1.OpenURL("http://mundosdelsur.hol.es/ip.txt")
        port = 7666 ''Val(frmCargando.Inet1.OpenURL("http://mundosdelsur.hol.es/port.txt"))
    #Else
        Ip = "LOCALHOST" ''"mdsao.ddns.net"
        port = 7666
    #End If
    InitGui
...
Public Sub CheckLauncherUpdate()
    If FileExist(App.path & "\Extras\LauncherNew.exe", vbNormal) = True Then
        Call Kill(App.path & "\Mundos del Sur.exe")
        Call FileCopy(App.path & "\Extras\LauncherNew.exe", App.path & "\Mundos del Sur.exe")
        If FileExist(App.path & "\Extras\LauncherNew.exe", vbNormal) = True Then
